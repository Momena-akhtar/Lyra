...
export class VoiceService {
   ...
   */
  async getUserTranscriptionStats(uid: string): Promise<{
    ...
    try {
      ...
      const transcriptions = transcriptionsSnapshot.docs.map(doc => doc.data() as TranscriptionResult);
  ...
  /**
   * Delete voice session and associated transcriptions
   */
  async deleteVoiceSession(sessionId: string, uid: string): Promise<void> {
    try {
      const sessionRef = db.collection('voice_sessions').doc(sessionId);
      const sessionDoc = await sessionRef.get();
